...
- **No instance variables**: Never use `$props`, `$emit`, `$attrs`, `$slots` - use setup-defined variables instead
...
```markdown
## Code Review: ProfileHero.vue changes
...
- **[src/components/ProfileHero.vue:45]** Missing error handling for image load failure. Consider adding `@error` handler.

### Suggestions

- **[src/components/ProfileHero.vue:23]** The computed `fullName` could be simplified using template literals.
```

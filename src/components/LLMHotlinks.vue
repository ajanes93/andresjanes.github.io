...
const openLLM = (provider: LLMProvider) => {
    // For Claude, we use a different approach since it supports URL params differently
    const url = `${provider.url}?${provider.promptParam}=${encodedPrompt.value}`
        ...
        <CardContent>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div v-for="provider in providers" :key="provider.id" class="relative group">
                    <Button
                        variant="outline"
                        class="w-full h-auto py-4 flex flex-col items-center gap-2 hover:border-primary/50 transition-all duration-300 group-hover:shadow-lg"
                        @click="openLLM(provider)"
                    >
                        <div
                    ...
                    <button
                        class="absolute -bottom-1 left-1/2 -translate-x-1/2 translate-y-full opacity-0 group-hover:opacity-100 transition-all duration-200 text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 py-1"
                        @click.stop="copyPrompt(provider.id)"
                    >

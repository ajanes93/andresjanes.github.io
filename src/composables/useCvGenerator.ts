...
export function useCvGenerator() {
    ...
    function generateCv({ profile }: CvGeneratorOptions): void {
        ...
        function addWrappedText(text: string, x: number, maxWidth: number, fontSize: number, color: [number, number, number] = textColor): number {
        ...
        doc.setFont('helvetica', 'normal')
        doc.setTextColor(...textColor)
        const languageStr = profile.languages.map((l: Language) => `${l.name} (${l.level})`).join(', ')
